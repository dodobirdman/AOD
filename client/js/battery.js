...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function reflectSessionBtn() { 
    if (sessionActive) {
      ...
      const currentInfoText = localStorage.getItem('sessionInfo') || (mode === 'smart' ? 
                              ...
                              document.getElementById('startLine')?.querySelector('strong')?.textContent);
      saveBtn.textContent = `Session active. ${currentInfoText || 'Details unavailable'}`;
    } else {
  ...
  async function updateSmart() {
    ...
    if (mins === Infinity && kWhReq > 0) {
        ...
        startLine.innerHTML = `Charging starts at <strong>--:--</strong> (check charge speed in settings)`;
        costLine.innerHTML = `Est. Cost: <strong>--.-- DKK</strong>`;
    } else if (mins === 0 && kWhReq === 0 && currentSoC >= tp) {
        startLine.innerHTML = `Already at or above target (<strong>${currentSoC.toFixed(0)}%</strong>).`;
        costLine.innerHTML = `Est. Cost: <strong>0.00 DKK</strong>`;
    } else if (mins === Infinity && kWhReq === 0) { 
        startLine.innerHTML = `Already at or above target. Charge speed is 0.`;
        costLine.innerHTML = `Est. Cost: <strong>0.00 DKK</strong>`;
    } else {
        ...
        if (!prices || prices.length < 24) {
            ...
            costLine.innerHTML = `Est. Cost: <strong>Price data unavailable</strong>`;
        } else {
  ...
  async function updateManual() {
    ...
    if (manStart.value && manEnd.value) {
        ...
        startLine.innerHTML = `Set start and end time for manual charging.`;
        costLine.innerHTML = `Est. Cost: <strong>--.-- DKK</strong>`;
    ...
    startLine.innerHTML = `Charge from ${manStart.value || '--:--'} to ${manEnd.value || '--:--'}. Est. new SOC: <strong>${newSoc.toFixed(0)}%</strong>`;
  ...
  saveBtn.addEventListener('click', () => { 
    ...
    if (sessionActive) {
      if (mode === 'smart') {
        ...
        sessionInfo = timeStrElement ? timeStrElement.textContent : 'N/A';
      } else {
        const socStrElement = startLine?.querySelector('strong');
        sessionInfo = socStrElement ? socStrElement.textContent : 'N/A';
      }
      localStorage.setItem('sessionInfo', sessionInfo);
    } else {
